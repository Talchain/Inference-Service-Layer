...
class StageAnalysis(BaseModel):
    ...
    stage_evpi: Optional[float] = Field(
        ...
        "IDENTIFIABILITY (F1, D-23.11): stage_evpi is EXACT (stage_evpi_status null) "
        "ONLY when at most ONE of the decision's actions faces resolvable chance "
        "ANYWHERE in its subtree — resolving that single uncertainty and re-choosing, "
        "every other action being deterministic, is an exact read of the tree. But "
        "when >=2 mutually-exclusive actions each have a chance node reachable in "
        "their subtree (immediate OR one or more levels deeper), the tree supplies "
        "only the MARGINAL distribution under each action and does NOT identify the "
        "JOINT law of outcomes across actions; the decide-after leg then ASSUMES those "
        "outcomes are independent across actions (it mixes one action's per-branch "
        "realised value against another's averaged value). That is one unrequested "
        "modelling choice — the same marginals also admit same-state or opposite "
        "coupling with different EVPIs — so the number is NOT exact for the supplied "
        "tree in that case (and where the deeper node is the SAME chance reused under "
        "another action, the tree identifies a different value than the emitted one). "
        "It is emitted with stage_evpi_status='assumed_independent_coupling' "
    ...
    stage_evpi_status: Optional[
        ...
        "identifiability). null/absent ONLY when stage_evpi is EXACT — meaning at most "
        "ONE of the decision's actions faces resolvable chance anywhere in its subtree "
        "(this includes a genuine EVPI of 0.0). Two null-value skip reasons: "
        ...
        "value-present disclosure: 'assumed_independent_coupling' (F1, D-23.11) — >=2 "
        "of the decision's actions each face chance reachable in their subtree "
        "(immediate or deeper), so stage_evpi is COMPUTED "
    ...
    coupling_assumption: Optional[Literal["independence_across_actions"]] = Field(
        ...
        "exclusive actions each facing a chance node reachable in its subtree "
        "(immediate OR deeper), so the decide-after leg mixed one action's per-branch "
        "realised value against another action's AVERAGED value — i.e. it ASSUMED the "
        "resolved chance is independent of the other action's (possibly deeper) "
        "uncertainty. The tree identifies only the marginals, not this joint law; the "
        "assumption is one modelling choice (the same marginals admit other couplings "
        "with different EVPIs, and a shared reused node identifies a different value "
        "than the emitted one) and is disclosed here rather than silently baked into a "
        "value labelled exact.",
    )
