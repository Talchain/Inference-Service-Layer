...
class RobustnessRequestV2(BaseModel):
    ...
    goal_threshold_frame: Optional[Literal["level", "delta"]] = Field(
        ...
        "quantity in the same normalised units as the graph's observed_state values; "
        "ISL converts it into the sample frame before comparing. When absent, or when "
        "a 'level' threshold cannot be converted, probability_of_goal is OMITTED and a "
        "structured inference warning names what was missing — ISL never guesses a "
        "frame and never emits a fabricated or clamped probability.",
    )
