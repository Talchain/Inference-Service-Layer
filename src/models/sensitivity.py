...
class CausalAssumption(BaseModel):
    ...
    name: str = Field(
        ..., description="Human-readable name of assumption", min_length=1, max_length=200
    )

    type: AssumptionType = Field(..., description="Type of assumption")

    description: str = Field(
        ..., description="Detailed explanation of what this assumption means", max_length=1000
    )

    violated_by: Optional[str] = Field(
        None, description="What would violate this assumption", max_length=500
    )

    testable: bool = Field(
        default=False, description="Whether this assumption can be empirically tested"
    )
...
class ViolationScenario(BaseModel):
    ...
    assumption_name: str = Field(..., description="Name of assumption being violated")

    severity: ViolationType = Field(..., description="How severe is this violation")

    magnitude: float = Field(
        ..., description="Quantitative measure of violation strength (0-1)", ge=0.0, le=1.0
    )

    description: str = Field(..., description="What this violation represents", max_length=500)

    parameters: Dict[str, float] = Field(
        default_factory=dict, description="Violation-specific parameters"
    )
...
class SensitivityMetric(BaseModel):
    ...
    assumption: str = Field(..., description="Name of assumption tested")

    baseline_outcome: float = Field(..., description="Predicted outcome under assumption")
    ...
    max_deviation_percent: float = Field(
        ..., description="Maximum percent deviation from baseline", ge=0.0
    )
    ...
    interpretation: str = Field(
        ..., description="Plain English explanation of sensitivity", max_length=500
    )

    violation_details: List[Dict[str, float]] = Field(
        default_factory=list, description="Detailed results for each violation tested"
    )
...
class SensitivityReport(BaseModel):
    ...
    least_critical: List[str] = Field(
        default_factory=list, description="Names of least critical assumptions"
    )
    ...
    summary: str = Field(
        ..., description="Plain English summary of sensitivity findings", max_length=1000
    )
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                ...
                "metadata": {"timestamp": "2025-11-23T12:00:00Z", "version": "1.0.0"},
            }
        }
    }
...
class SensitivityRequest(BaseModel):
    ...
    model: Dict = Field(..., description="Structural causal model specification")

    intervention: Dict[str, float] = Field(..., description="Intervention values to analyze")

    assumptions: List[str] = Field(
        ..., description="List of assumption types to test", min_length=1, max_length=10
    )
    ...
    n_samples: int = Field(
        default=100, description="Number of samples for stochastic violations", ge=10, le=1000
    )
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                "model": {
                    "type": "linear",
                    "equations": {"Revenue": "100 * Price + 5000 * Quality - 200"},
                },
