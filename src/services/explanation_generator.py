...
class ProgressiveExplanation(BaseModel):
    ...
    headline: str = Field(
        ..., description="Simple, non-technical headline (1 sentence)", max_length=200
    )
    ...
    details: str = Field(..., description="Technical details for experts", max_length=1000)

    visual_aid: Optional[str] = Field(None, description="Suggested visualization type")

    learn_more_url: Optional[str] = Field(None, description="Link to documentation")
...
class ExplanationGenerator:
    ...
    def generate_progressive(self, concept: str, data: Dict) -> ProgressiveExplanation:
        """
    ...
    def validate_explanation_quality(
        self, explanation: str, target_level: Literal["simple", "intermediate", "technical"]
    ) -> Dict[str, Any]:
        """
        ...
        flesch_ok = (
            target["flesch_reading_ease"][0]
            <= scores["flesch_reading_ease"]
            <= target["flesch_reading_ease"][1]
        )
        grade_ok = (
            target["flesch_kincaid_grade"][0]
            <= scores["flesch_kincaid_grade"]
            <= target["flesch_kincaid_grade"][1]
        )
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                scores, target_level, targets[target_level]
            ),
        }

    def _generate_recommendations(
        self, scores: Dict, target_level: str, target_ranges: Dict
    ) -> List[str]:
        ...
        if scores["flesch_reading_ease"] < target_ranges["flesch_reading_ease"][0]:
            recommendations.append(
                "Text is too difficult. Use simpler words and shorter sentences."
            )
        ...
        if scores["words_per_sentence"] > target_ranges["max_words_per_sentence"]:
            recommendations.append(
                f"Sentences are too long (avg {scores['words_per_sentence']:.1f} words). Break into shorter sentences."
            )
    ...
    def generate_counterfactual_explanation(
        ...
        intervention_str = ", ".join(f"{var}={val}" for var, val in intervention.items())
    ...
    def generate_causal_validation_explanation(
        ...
        if status == "identifiable":
            ...
            simple_explanation = (
                f"We can estimate this causal effect by controlling for {minimal_str}."
            )
            ...
            simple_explanation = (
                "We cannot reliably estimate this causal effect due to confounding."
            )
            ...
            simple_explanation = (
                "We completed the analysis but with reduced confidence in the results."
            )
